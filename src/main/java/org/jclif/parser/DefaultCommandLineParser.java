...
class DefaultCommandLineParser extends CommandLineParser {
  ...
  private static final Pattern PARAM_VALUE_REGEX =
      Pattern
          .compile("((\"[\\p{Alnum}\\p{Punct}\\p{Space}&&[^\"]]+\")|('[\\p{Alnum}\\p{Punct}\\p{Space}&&[^']]+')|([\\p{Alnum}\\p{Punct}]+))");

  @Override
  public CommandLineParseResult parse(CommandLineConfiguration configuration, String... args)
    ...
    if (!cmdFound) {
      parseOptions(scanner, configuration, null, configuration.getOptionConfiguration(),
          resultSet.getOptionInput());
      parseParameters(scanner, null, configuration.getParameterConfiguration(),
          resultSet.getParameterInput());
    }
    scanner.close();
  ...
  private boolean parseCommand(Scanner scanner, CommandLineConfiguration configuration,
    ...
    if (paramsToken != null) {
      ...
      parseOptions(scannerOptions, configuration, cmdMetadata,
          cmdMetadata.getOptionConfigurations(), resultSet.getOptionInput());
      parseParameters(scannerOptions, cmdMetadata, cmdMetadata.getParameterConfigurations(),
          resultSet.getParameterInput());
  ...
  private boolean parseOptions(Scanner scanner, CommandLineConfiguration config,
      CommandMetadata cmdMetadata, OptionConfiguration optionConfig, OptionInputSet resultSet)
      throws InvalidInputException {
    ...
    while (true) {
      ...
      try {
        ...
        OptionMetadata metadata =
            extractOptionMetadata(config.getCommandLineProperties(), optionConfig, optionPrefix,
                optionId);
        Object parameterValue =
            extractParameterValue(scanner, config, cmdMetadata, metadata, optionId, optionPrefix);
  ...
  private OptionMetadata extractOptionMetadata(CommandLineProperties cmdLineProperties,
      OptionConfiguration optionConfig, String optionPrefix, String optionId)
      throws InvalidInputException {
    ...
    if (cmdLineProperties.getOptionPrefix().equals(cmdLineProperties.getOptionLongPrefix())) {
      ...
      metadata =
          optionConfig.getOption(optionId, (longPrefix) ? IdentifierType.LONG
  ...
  private String[] extractOptionStrings(Scanner scanner, CommandLineProperties cmdLineProperties) {
    ...
    // parse option identifier extract option identifier and get its
    // metadata
  ...
  private Object extractParameterValue(Scanner scanner, CommandLineConfiguration config,
      CommandMetadata cmdMetadata, OptionMetadata metadata, String optionId, String optionPrefix)
      throws InvalidInputException {
    ...
    if (metadata.isParameterAccepted() && token != null) {
      ...
      if (parameterMetadata.isRequired() && token == null) {
        throw new InvalidInputException("Parameter required for option " + optionPrefix + optionId
            + ".", cmdMetadata);
      }
      ...
      if (token != null) {
        ...
        for (int i = 0; i < paramResult.groupCount(); i++) {
          LOGGER.info(String.format("[%d] Delim={%s}, Param={%s}", i, config
              .getCommandLineProperties().getOptionParameterDelim(), paramResult.group(i)));
        }
        String groupValue = StringUtil.extractQuotedValue(paramResult.group(1));
        LOGGER.info(String.format("Delim={%s}, Param={%s}", config.getCommandLineProperties()
            .getOptionParameterDelim(), groupValue));
  ...
  private Object getParameterValue(CommandMetadata cmdMetadata, ParameterMetadata metadata,
      String paramValue) throws InvalidInputException {
    try {
      ...
      throw new InvalidInputException("Invalid parameter value '" + paramValue + "' for parameter "
          + metadata.getIdentifier(), e, cmdMetadata);
    }
  }

  private boolean parseParameters(Scanner scanner, CommandMetadata cmdMetadata,
      ParameterConfiguration parameterConfig, ParameterInputSet resultSet)
      throws InvalidInputException {
  ...
  private List<Object> extractMultiValuedParameter(Scanner scanner, CommandMetadata cmdMetadata,
    ...
    if (paramMeta.isRequired() && valueList.isEmpty()) {
      throw new InvalidInputException("Parameter " + paramMeta.getIdentifier()
          + " is required but not specified", cmdMetadata);
    }
  ...
  @Override
  public boolean matches(CommandLineConfiguration configuration, CommandLineParseResult resultSet,
      String... args) {
    try {
  ...
  void validate(CommandLineConfiguration configuration, CommandLineParseResult result)
      throws InvalidInputException {
  ...
  private void validateOptions(CommandLineConfiguration configuration,
      OptionConfiguration optionConfig, CommandLineParseResult result) throws InvalidInputException {
  ...
  private void validateParameters(CommandLineConfiguration configuration,
      ParameterConfiguration parameterConfig, CommandLineParseResult result)
      throws InvalidInputException {
