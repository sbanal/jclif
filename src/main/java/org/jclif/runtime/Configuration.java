...
/**
 * This class represents a configuration used by the executor when processing command line inputs. A
 * Configuration contains the properties which defines the package handler name, command handler
 * list and application name.
 * 
...
public class Configuration extends Properties {
  ...
  /**
   * Loads configuration properties from default configuration input stream and loads handler list.
   * 
  ...
  public static InputStream getDefaultConfigInputStream() throws IOException {
    InputStream stream =
        Executor.class.getClassLoader().getResourceAsStream(DEFAULT_EXECUTOR_CONFIG_FILE);
    if (stream == null) {
      String configFileName =
          System.getProperty(PROPERTY_JCLIF_CONFIG_FILE, DEFAULT_EXECUTOR_CONFIG_FILE);
      String configDirPath =
          System.getProperty(PROPERTY_JCLIF_INSTALL_PATH, System.getProperty("user.dir"));
      File configFile = new File(new File(configDirPath).getCanonicalFile(), configFileName);
      stream = new FileInputStream(configFile);
      LOGGER.info(String.format("Executor using file config file '%s'",
          configFile.getCanonicalPath()));
    } else {
      LOGGER.info(String.format("Executor using resource config file '%s'",
          DEFAULT_EXECUTOR_CONFIG_FILE));
    }
    return stream;
  }
  ...
  /**
   * Retrieves command handlers registered in the properties file. Returned list contains all
   * classes which are registered under properties org.jclif.app.handler.[index] in properties file.
   * 
  ...
  public List<Class<?>> getHandlerList() {
    ...
    for (int i = 1; i < MAX_HANDLERS; i++) {
      ...
      try {
        ...
        LOGGER.log(Level.WARNING, "Handler class " + className + " not found in classpath", e);
      }
    }
  ...
  void extractJarHandlerList(JarURLConnection conn, String appMainPackage,
      List<Class<?>> packageHandlerList) throws IOException {
    ...
    for (Enumeration<JarEntry> e = conn.getJarFile().entries(); e.hasMoreElements();) {
      ...
      if (entry.getName().startsWith(appMainPackage) && !entry.isDirectory()
  ...
  void extractClassPackages(File srcDir, String appMainPackage, List<Class<?>> packageClasses)
      throws IOException {
    ...
    for (File file : files) {
      ...
      if (file.isDirectory()) {
        ...
        if (entryFilePath.endsWith(appMainPackage + File.separator + file.getName())
          ...
          try {
            Class<?> classType =
                Class.forName(StringUtil.pathToClassName(appMainPackage + "." + file.getName()));
            packageClasses.add(classType);
          } catch (ClassNotFoundException e) {
            // Skipping those classes which cannot be found in
            // classpath!
            LOGGER.log(Level.WARNING, "Loading Handler from local path failed for " + file, e);
          }
        }
      }
