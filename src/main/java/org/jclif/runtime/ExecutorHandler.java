...
public class ExecutorHandler {
  ...
  private static final Logger LOGGER = Logger.getLogger(ExecutorHandlerRegistry.class
      .getCanonicalName());
  ...
  public ExecutorHandler(Class<?> handlerClass) {
    this(AnnotationProcessor.getHandlerMetadata(handlerClass), handlerClass, AnnotationProcessor
        .getHandlerMethod(handlerClass));
  }
  ...
  /**
   * Executes a handler. Execution will create a new instance of the handler class then populates
   * the handler fields right before executing the handler method.
   * 
  ...
  public void execute(CommandLineParseResult result) {
    ...
    try {
      ...
      CommandMetadata cmdMetadata =
          (result.isCommandMatch()) ? (CommandMetadata) result.getMatchingCommand().getMetadata()
              : null;
      OptionConfiguration optionConfig =
          (cmdMetadata == null) ? result.getConfiguration().getOptionConfiguration() : cmdMetadata
   ...
   */
  Object getHandlerFieldInputValue(Field field, OptionConfiguration optionConfig,
  ...
  void invokeSetterMethod(Field field, Object handlerInstance, Object value) {
    ...
    try {
      LOGGER.fine(String.format("Calling set method for field " + field.getName() + ", value = "
          + value));
      ...
      throw new UnsupportedOperationException("Unable to execute" + " setter method "
          + handlerClass.getCanonicalName() + " for field " + field.getName() + " with param "
