...
public final class AnnotationProcessor {
  ...
  public static CommandMetadata getHandlerMetadata(Class<?> commandHandler) {
    ...
    if (commandAnnotation == null) {
      throw new IllegalArgumentException("Unable to extract annotation from " + commandHandler
          + ". Skipping processing.");
    }
    ...
    for (Field field : commandHandler.getDeclaredFields()) {
      ...
      if (field.isAnnotationPresent(Option.class)) {
        ...
        ParameterMetadata parameter =
            new ParameterMetadataImpl(optionAnnotation.identifier(), paramType);
    ...
    CommandMetadata metadata =
        new CommandMetadataImpl(commandAnnotation.identifier(), optionConfig, parameterConfig,
            commandAnnotation.description(), commandAnnotation.longDescription());
    return metadata;
  }
