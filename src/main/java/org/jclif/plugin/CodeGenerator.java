...
public class CodeGenerator {
  ...
  public void generateMain(String commandAnnotatedPackage, String targetDir) throws IOException {
    ...
    // scan all classes of a directory and detect which ones are JCLIF
    // annotated
  ...
  private Configuration createHandlerConfiguration(File[] files, String commandAnnotatedPackage) {
    ...
    for (File srcFile : files) {
      String className =
          commandAnnotatedPackage + "." + StringUtil.sourceNameToClassName(srcFile.getName());
      try {
        ...
        LOGGER.log(Level.WARNING,
            String.format("Unable to load class %s due to error '%s'", className, e.getMessage()),
            e);
      } catch (Exception e) {
        LOGGER.log(Level.WARNING,
            String.format("Unable to load class %s due to error '%s'", className, e.getMessage()),
  ...
  private File[] getClassSourceFiles(File confSourceDir, String commandAnnotatedPackage)
      throws IOException {

    File classSourceDir =
        new File(confSourceDir, commandAnnotatedPackage.replace(".", File.separator));
    ...
    if (files == null || 0 == files.length) {
      throw new IllegalArgumentException("No Java files found in directory "
          + classSourceDir.getCanonicalPath());
    }
