...
/**
 * This class is a collection class used to store command line input configuration metadata
 * information. Class is a LinkedHashMap<String, InputMetadata> type and ensures that only one
 * metadata exist in a collection. Class also ensures that the metadeta identifiers are valid and
 * does not exist in the collection.
 * 
 * Any metadata added to the collection which has an invalid identifier or currently exist in the
 * collection will trigger a InvalidIdentifierException from being thrown.
 * 
...
public abstract class Configuration<T extends InputMetadata> extends LinkedHashMap<String, T> {
  ...
  public T add(T metadata) {
    LOGGER.info("Add identifier: " + metadata.getIdentifier() + ", metadata-desc:"
        + metadata.getDescription());
    validateIdentifier(metadata.getIdentifier(), true);
    if (null != get(metadata.getIdentifier())) {
      throw new InvalidIdentifierException(this.getId() + " identifer '" + metadata.getIdentifier()
          + "' already exist.");
    }
  ...
  public void putAll(Map<? extends String, ? extends T> m) {
    throw new UnsupportedOperationException(
        "Operation not supported, use boolean addAll(Collection<T> metadataList) instead.");
  }
